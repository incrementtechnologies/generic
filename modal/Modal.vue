      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group d-flex flex-column" v-bind:class="(item.row !== 'full' ? item.row + ' float-left' : '')" v-for="(item, index) in property.inputs" :key="index">
            ...
            <label class="text-danger" v-bind:for="item.id" v-if="(item.type === 'input' || item.type === 'textarea') && item.value !== '********' && item.value !== null && item.validation.type === 'text' && (item.validation.size > item.value.length)" style="float: left; width: 100%;"><b>Oops!</b> Length must be greater than or equal to {{item.validation.size}}.</label>
